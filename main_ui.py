...
_NET_COLLECTOR_FILENAME = "network_collector_windows.py" # Script name remains the same
...
class SecurityMonitorApp(ctk.CTk):
    def __init__(self):
        ...
        self.geometry("1200x800") # Slightly wider for new input field
        ...
        self.grid_rowconfigure(1, weight=0) # Control Panel Row 2 for Interface input
        self.grid_rowconfigure(2, weight=1) # Output Display
        self.grid_rowconfigure(3, weight=0) # Status Bar
        self.grid_columnconfigure(0, weight=1)

        # --- Top Row: Control Panel (Primary Buttons) ---
        ...
        # Configure columns for buttons - adjust as needed
        self.control_panel_row1.grid_columnconfigure((0,1,2,3,4,5), weight=0)
        self.control_panel_row1.grid_columnconfigure(6, weight=1) # Spacer
        ...
        self.clear_button.grid(row=0, column=5, padx=(5,10), pady=5)
        ...
        self.control_panel_row2.grid_columnconfigure(0, weight=0) # Label
        self.control_panel_row2.grid_columnconfigure(1, weight=1) # Entry

        self.tshark_interface_label = ctk.CTkLabel(self.control_panel_row2, text="TShark Interface (e.g., '3' or name, optional):")
        self.tshark_interface_label.grid(row=0, column=0, padx=(5,2), pady=5, sticky="w")

        self.tshark_interface_entry = ctk.CTkEntry(self.control_panel_row2, placeholder_text="Leave blank for auto-detect")
        self.tshark_interface_entry.grid(row=0, column=1, padx=(0,10), pady=5, sticky="ew")


        # --- Middle Row: Output Display (CTkTabview) ---
        self.tab_view = ctk.CTkTabview(self, corner_radius=8)
        self.tab_view.grid(row=2, column=0, sticky="nsew", padx=10, pady=5) # Adjusted row
        ...
        self.tab_view.add("Network Traffic (TShark)") # Updated Tab Name
        ...
        self.status_bar.grid(row=3, column=0, sticky="ew", padx=10, pady=(5,10)) # Adjusted row
    ...
    def start_collection_logic(self):
        ...
        self.save_net_log_button.configure(state=tk.DISABLED)
        self.save_both_button.configure(state=tk.DISABLED)
        self.tshark_interface_entry.configure(state=tk.DISABLED) # Disable while running
        ...
        try:
            ...
            tshark_interface_val = self.tshark_interface_entry.get().strip()
            if tshark_interface_val:
                net_cmd.extend(["--interface", tshark_interface_val]) # Use --interface as defined in network_collector

            self.net_collector_proc = subprocess.Popen(
                net_cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, bufsize=1,
                creationflags=subprocess.CREATE_NO_WINDOW
            )
            threading.Thread(target=self._reader_thread, args=(self.net_collector_proc, self.net_output_queue, "Network Collector (TShark)"), daemon=True).start() # Updated name
            threading.Thread(target=self._log_subprocess_stderr, args=(self.net_collector_proc, self.net_output_queue, "Network Collector (TShark)"), daemon=True).start() # Updated name
    ...
    def stop_collection_logic(self, force_ui_update=False):
        ...
        self.monitoring_active = False
        self.tshark_interface_entry.configure(state=tk.NORMAL) # Re-enable entry
        ...
        # Logic for enabling save buttons remains the same
    ...
    def save_log(self, log_type):
        ...
        if log_type == "host":
            ...
            default_filename = f"network_traffic_tshark_{datetime.datetime.now():%Y%m%d_%H%M%S}.txt" # Updated name
        else: return
    ...
    def save_both_logs(self):
        ...
        if host_content:
            file_path_host = filedialog.asksaveasfilename(
                ...
                title="Save Host Events Log As...", defaultextension=".csv", filetypes=[("CSV files", "*.csv")]
            )
            if file_path_host:
                try:
                ...
                if not messagebox.askyesno("Continue?", "Host log saving cancelled. Continue to save network log?"): return
        else: messagebox.showinfo("Save Both", "No host events to save.")

        net_content = self.net_output_textbox.get("1.0", tk.END).strip()
        if net_content:
            file_path_net = filedialog.asksaveasfilename(
                initialfile=f"network_traffic_tshark_{datetime.datetime.now():%Y%m%d_%H%M%S}.txt", # Updated name
        ...
        else: messagebox.showinfo("Save Both", "No network traffic (TShark) to save.") # Updated name
